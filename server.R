...
shinyServer(function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    else ({ p <- ggplot(data=Baseball %>% filter(franchID == input$team) %>% 
                                                filter(yearID >= input$year[1], 
                                                       yearID <= input$year[2]), 
                                               aes(x=.data[[input$xvar]],y=.data[[input$yvar]])) + 
        geom_point(size=4, aes(color=franchID)) 
    
      if(input$lines == 1) p <- p + geom_smooth(se=FALSE)  + facet_wrap(vars(franchID))
    
      if(input$lines == 2) p <- p + geom_smooth(method="lm", se=FALSE) + facet_wrap(vars(franchID))
